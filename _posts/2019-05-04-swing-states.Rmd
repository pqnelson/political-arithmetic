...
new_train_data <- data[which(data$state == 'Texas'),]
train_data <- rbind(train_data,new_train_data);
...
train_data <- rbind(train_data,new_train_data);
new_train_data <- data[which(data$state == 'Arizona'),]
train_data <- rbind(train_data,new_train_data);
...
logit_mod <- bayesglm(is_swing ~ swings + margin + bellwether + swings*margin + swings*bellwether + bellwether*margin + swings*margin*bellwether, data = train_data, family = binomial(link = "logit"));
```

Using just the number of swings, the margin of victory, and if it's a bellwether state, we have the following predictions:

```{r}
predicted <- predict(logit_mod, data, type="response")
data$predicted <- predicted
kable(data[order(-data$predicted),c('year','state','predicted')], row.names = FALSE)
```
