...
pool_polls <- function(data, end_date) {
  ...
  if (nrow(data) == 1) {
    ...
    cov_mat <- 2*multinomial_covariance_mat(matrix(data = prob_data, ncol=1), data$sample_size) + ifelse(is.na(data[[1, "predictive"]]),
                                                                                                                                                          diag(data$sample_size, 4), 
                                                                                                                                                          sqrt(data$sample_size)*diag(data[[1, "predictive"]]/100,4))
    ...
    for (i in 1:nrow(data)) {
      ...
      prob_data <- matrix(c(data[[i,'p_dem']],data[[i,'p_rep']],data[[i,'p_third']],data[[i,'p_undecided']]), ncol=1) + use_house_effect*c(parse_house_effect(data[[i,'house_effect']]))# /sqrt(size)
      covariance_mat <- 2*multinomial_covariance_mat(matrix(data = prob_data, ncol=1), size) +
                            ifelse(is.na(data[[i, "predictive"]]),
                                   diag(sqrt(size),4),
                                   sqrt(size)*diag(data[[i, "predictive"]]/100,4))
      p_mat <- MASS::ginv(covariance_mat)
      prob_data <- matrix(c(data[[i,'p_dem']],data[[i,'p_rep']],data[[i,'p_third']],data[[i,'p_undecided']]), ncol=1) # + use_house_effect*parse_house_effect(data[[i,'house_effect']])#/sqrt(size)
    ...
    mu <- cov_mat %*% mu_numerator
    prob <- cov_mat %*% prob_numerator
    size <- sum(mu)
...
run_2020 <- function(end_date = today(), is_nowcast = F) {
  results <- list();
  for (state_name in unique(polls_2020$state)) {
    if (state_name != "--") {
      # print(state_name)
      polls <- norm_approx(polls_2020 %>%
                             filter(state == state_name,
                                    end.date <= end_date),
                           rep=trump, dem=biden);
...
outfile <- sprintf("../img/2020-plots/kalman-filter-%s-hypo.png",date_run)
