...
parse_house_effect <- function(house_effect) {
  ...
  if (is.numeric(house_effect)) { return(matrix(c(0,0,0,0), ncol=1)); }
...
expected_ev <- function(forecast) {
  ...
  for (state in names(forecast)) {
    if (is.na(forecast[[state]])) {
      ...
      # acc[["dem"]] <- acc[["dem"]] + forecast[[state]]*electoral_delegates.abbrev[[state]];
      # acc[["rep"]] <- acc[["rep"]] + (1-forecast[[state]])*electoral_delegates.abbrev[[state]];
...
pollster_ratings_2020 <- read_csv("../data/polling/ratings_2020.csv") %>%
  mutate(banned = ifelse(Pollster == "Trafalgar Group", "yes", `Banned by 538`)) %>%
  mutate(`Banned by 538` = banned)
```
...
election_date_2022 <- ymd("2022-11-08")
...
chart_support_comparison_over_time <- function(prob_16, prob_20, state_abbrev, rnc_end_date = ymd('2020/08/27')) {
  ...
  ggplot(data=probabilities_df,
    ...
    annotate("text",
             ...
             label="\nRNC ends",
             angle=90, size=3.25) +
    geom_vline(xintercept=as.numeric(rbg), linetype=4) +
    annotate("text",
             x=(rbg), y=25,
             label="\nRBG dies",
             angle=90, size=3.25) +
...
```{r}
kable(polls_2020 %>% filter(#entry.date.time..et. >= today() - weeks(2)
                            # start.date + 0.5*(end.date - start.date)
                            #entry.date.time..et. >= today() - days(3)
                            end.date >= today() - days(3)) %>% group_by(state) %>% summarize(count = n()) %>% arrange(-count))
```
...
```{r}
tail(p20_all %>% filter(state == "TX"))
save_comparison_plot(p16_all, p20_all, "TX")
```

```{r}
tail(p20_all %>% filter(state == "PA"))
save_comparison_plot(p16_all, p20_all, "PA")
```
...
```{r}
tail(p20_all %>% filter(state == "NC"))
save_comparison_plot(p16_all, p20_all, "NC")
```

```{r}
tail(p20_all %>% filter(state == "SC"))
save_comparison_plot(p16_all, p20_all, "SC")
```



```{r}
tail(p20_all %>% filter(state == "AZ"))
save_comparison_plot(p16_all, p20_all, "AZ")
```
...
```
```{r}
inv_logit <- function(odds) {
  odds/(1 + odds)
}
tail(p20_all %>% filter(state == "OH"))
save_comparison_plot(p16_all, p20_all, "OH")
```

```{r}
tail(p20_all %>% filter(state == "MI"))
save_comparison_plot(p16_all, p20_all, "MI")
```

```{r}
tail(p20_all %>% filter(state == "WI"))
...
FiveThirtyEight has provided the Senate polls, so let's pull them down.

```{r}
download_latest_senate_polls <- function() {
  senate_polls_url <- "https://projects.fivethirtyeight.com/polls/data/senate_polls.csv" # "https://projects.fivethirtyeight.com/polls-page/senate_polls.csv"
  if(file.exists("../data/polling/senate/senate_polls.csv")) {
    file.rename("../data/polling/senate/senate_polls.csv",
                paste0("../data/polling/senate/senate_polls.",
                       as.numeric(now()),
                       ".csv"))
  }
  download.file(senate_polls_url, 
                "../data/polling/senate/senate_polls.csv", method = "auto")
}
download_latest_senate_polls()
```

We have to transform them to a format suitable for Kalman filtering.

```{r}
senate_raw <- read_csv("../data/polling/senate/senate_polls.csv") %>%
  mutate(pollster = recode(pollster, 
                           `WPA Intelligence` = "WPA Intelligence (WPAi)",
                           `Mellman` = "Mellman Group",
                           `Selzer` = "Selzer & Co.",
                           `Clarity` = "Clarity Campaign Labs",
                           `Susquehanna` = "Susquehanna Polling & Research Inc.",
                           `Garin Hart Yang` = "Garin-Hart-Yang Research Group",
                           `Braun` = "Braun Research",
                           `Remington` = "Remington Research Group",
                           `Fabrizio` = "Fabrizio, Lee & Associates",
                           `Tarrance` = "Tarrance Group",
                           `U. Georgia SPIA` = "University of Georgia School of Public and International Affairs",
                           `Quinnipiac` = "Quinnipiac University",
                           `PPP` = "Public Policy Polling",
                           `Mason-Dixon` = "Mason-Dixon Polling & Strategy",
                           `U. North Florida` = "University of North Florida",
                           `Saint Leo U.` = "Saint Leo University",
                           ...
                           `OH Predictive Insights / MBQF` = "OH Predictive Insights",
                           `U. New Hampshire`="University of New Hampshire",
                           `Trafalgar Group/InsiderAdvantage` = "Trafalgar Group"))

trim_for_state <- function(senate_polls, state_fullname) {
  
  trimmed_senate <- senate_polls %>% 
    filter(state == state_fullname) %>%
    rename(number.of.observations = sample_size) %>%
    mutate(end.date = (end_date),
           start.date = (start_date)) %>%
    dplyr::select(-start_date, -end_date,
                  -display_name,-sponsor_ids,-sponsors,
                  -pollster_rating_name, -population_full, -office_type, 
                  -seat_number, -seat_name, -created_at, -url, 
                  -stage, -race_id, -candidate_id, -candidate_party, 
                  -nationwide_batch, -tracking, -partisan, -internal, 
                  -pollster_rating_id, -methodology, -election_date, 
                  -sponsor_candidate, -ranked_choice_reallocated, 
                  -notes, -answer) %>%
    spread(candidate_name, pct) %>%
    arrange(end.date)
  trimmed_senate
}
...
Let us now try to find the normal approximation for Iowa's senate race.

```{r}
ia_2020 <- full_kalman(norm_approx(trim_for_state(senate_raw, "Iowa") %>%
                                     filter(!is.na(`Theresa Greenfield`),
                                            !is.na(`Joni K. Ernst`)) %>%
                                     rename(joni_ernst = `Joni K. Ernst`,
                                            theresa_greenfield = `Theresa Greenfield`) %>%
                                     mutate(undecided = NA,
                                            other = NA,
                                            house_effect = "D+0",
                                            predictive=0) %>%
                                     dplyr::select(-`Eddie Mauro`, -`Rick Stewart`, -`Suzanne Herzog`),
                                   joni_ernst, theresa_greenfield), election_date_2020)
```

We can try to generalize this further:

```{r}
kalman_senate <- function(polls, state_name, election_date) {
  state_polls <<- polls %>%
    filter(state == state_name) %>% 
    mutate(end_date = mdy(end_date),
           start_date = mdy(start_date))
  republican <<- filter(state_polls, candidate_party == "REP") %>%
    arrange(end_date) %>% pull(candidate_name) %>% 
    last 
  democrat <<- filter(state_polls, candidate_party == "DEM") %>%
    arrange(end_date) %>% pull(candidate_name)  %>% 
    last
  
  poll_df <<- state_polls %>% 
    trim_for_state(state_name) %>%
  ...
  full_kalman(norm_approx(poll_df, dem=democrat, rep=republican), election_date)
}
```

```{r}
transform_senate_result <- function(kalman_senate, state_name) {
  results <- data.frame(state = c(),
                        democrat = c(),
                        republican = c(),
                        other = c(),
                        undecided = c(),
                        date = c());
  for (i in 1:length(kalman_senate$prob)) {
    row <- kalman_senate$prob[[i]]
    results <- rbind(results,
                     tibble(state = state_name,
                            democrat = row[[1]],
                            republican = row[[2]],
...
```{r}
save_senate_plot <- function(senate_data, state_name, rnc_end_date = ymd('2020/08/27'), as_of_date=today()) {
  rbg <- ymd('2020/09/18')
  plot <- ggplot(data=senate_data %>%
                   transform_senate_result(state_name) %>%
                   gather("party", "percent", republican, democrat, undecided, third_party) %>%
                   mutate(percent = 100*percent,
                          party = factor(party, c("democrat", "republican", "undecided", "third_party")),
                          days = date),
                 aes(x=days, y=percent, group=party)) +
    #geom_vline(xintercept=as.numeric(rnc_end_date), linetype=4) +
    #annotate("text",
    ...
    scale_color_manual(values=c("#0015BC", "red", "#daa520", "purple")) +
    scale_x_date(date_labels = "%b",
                 date_breaks = "1 months") +
    labs(title = paste0(state_name, " Senate Polls (2022)"),
         subtitle = paste0("State-level Senate polls, Kalman filtered, ",
                           format(as_of_date, "%b %d, %Y")),
         caption = "https://political-arithmetic.blogspot.com")
  
  
  # plot <- chart_support_comparison_over_time(prob_16, prob_20 %>% filter(date <= date_run), state_abbrev)
  outfile <- sprintf("../img/2020-plots/senate-comparison-%s-%s.png", state_name, as_of_date)
  ggsave(outfile, plot=plot, height=4, dpi=500)
  plot
}
```
...
```{r}
foo <- senate_raw %>% rename(candidate_party = party)
oh_senate <- kalman_senate(foo, "Ohio", election_date_2022)
save_senate_plot(oh_senate, "Ohio")
```


```{r}
ga_senate <- kalman_senate(foo, "Georgia", election_date_2022)
save_senate_plot(ga_senate, "Georgia")
```

```{r}
fl_senate <- kalman_senate(foo, "Florida", election_date_2022)
save_senate_plot(fl_senate, "Florida")
```

```{r}
az_senate <- kalman_senate(foo, "Arizona", election_date_2022)
save_senate_plot(az_senate, "Arizona")
```

```{r}
wi_senate <- kalman_senate(foo, "Wisconsin", election_date_2022)
save_senate_plot(wi_senate, "Wisconsin")
```

```{r}
nh_senate <- kalman_senate(foo, "New Hampshire", election_date_2022)
save_senate_plot(nh_senate, "New Hampshire")
```


```{r}
nv_senate <- kalman_senate(foo, "Nevada", election_date_2022)
save_senate_plot(nv_senate, "Nevada")
```


```{r}
ky_senate <- kalman_senate(senate_raw, "Kentucky", election_date_2020)
save_senate_plot(ky_senate, "Kentucky")
```

```{r}
nc_senate <- kalman_senate(senate_raw, "North Carolina", election_date_2020)
transform_senate_result(nc_senate, "North Carolina")
save_senate_plot(nc_senate, "North Carolina")
```

```{r}
sc_senate <- kalman_senate(senate_raw, "South Carolina", election_date_2020)
transform_senate_result(sc_senate, "South Carolina")
save_senate_plot(sc_senate, "South Carolina")
```



```{r}
tx_senate <- kalman_senate(senate_raw, "Texas", election_date_2020)
tail(transform_senate_result(tx_senate, "Texas"))
save_senate_plot(tx_senate, "Texas")
```


```{r}
mt_senate <- kalman_senate(senate_raw, "Montana", election_date_2020)
(transform_senate_result(mt_senate, "Montana"))
save_senate_plot(mt_senate, "Montana")
```


```{r}
mi_senate <- kalman_senate(senate_raw, "Michigan", election_date_2020)
tail(transform_senate_result(mi_senate, "Michigan"))
save_senate_plot(mi_senate, "Michigan")
```


```{r}
ga_senate <- kalman_senate(senate_raw %>% filter(race_id == 6271), "Georgia", election_date_2020)
tail(transform_senate_result(ga_senate, "Georgia"))
save_senate_plot(ga_senate, "Georgia")
```

```{r}
nc_senate <- kalman_senate(senate_raw %>%
                             mutate(end = parse_date(end_date,"%m/%d/%y"),
                                    created = parse_datetime(created_at, "%m/%d/%y %H:%M")) %>%
                             filter(as.Date(created) <= as.Date('2020-10-01'),
                                    end <= as.Date('2020-10-01')),
                           "North Carolina",
                           election_date_2020)
tail(transform_senate_result(nc_senate, "North Carolina") %>% filter(date < as.Date('2020-10-28')))
# save_senate_plot(nc_senate, "North Carolina", as_of_date=as.Date('2020-10-01'))
```

```{r}
az_senate <- kalman_senate(senate_raw %>% filter(created <= as.Date('2020-10-27'), end_date <= as.Date('2020-10-27')), "Arizona", election_date_2020)
tail(transform_senate_result(az_senate, "Arizona"))
save_senate_plot(az_senate, "Arizona")
```

```{r}
ak_senate <- kalman_senate(senate_raw, "Alaska", election_date_2020)
save_senate_plot(ak_senate, "Alaska")
```
```{r}
me_senate <- kalman_senate(senate_raw, "Maine", election_date_2020)
transform_senate_result(me_senate, "Maine")
save_senate_plot(me_senate, "Maine")
```
```{r}
ia_senate <- kalman_senate(senate_raw, "Iowa", election_date_2020)
tail(transform_senate_result(ia_senate, "Iowa"))
save_senate_plot(ia_senate, "Iowa")
```

```{r}
co_senate <- kalman_senate(senate_raw, "Colorado", election_date_2020)
tail(transform_senate_result(co_senate, "Colorado"))
save_senate_plot(co_senate, "Colorado")
```
