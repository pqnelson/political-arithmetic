...
```{r}
pool_single_poll <- function(data, end_date) {
  min_probability <- qnorm(0.975)/sqrt(data$sample_size)
  ...
  tmp <- 2*multinomial_covariance_mat(matrix(data = ifelse(prob_data<1e-6,qnorm(0.975)/sqrt(data$sample_size), prob_data), ncol=1), data$sample_size) +
      diag(sqrt(data$sample_size)*ifelse(is.na(data[[1, "predictive"]]),1,data[[1, "predictive"]])/100,4)
  
  cov_mat <- make_positive_definite(make_nonsingular(2*multinomial_covariance_mat(matrix(data = prob_data, ncol=1), data$sample_size), data$sample_size) +
      diag(sqrt(data$sample_size)*ifelse(is.na(data[[1, "predictive"]]),1,data[[1, "predictive"]])/100,4)) +
...
```{r}
pool_multiple_polls <- function(data, end_date) {
  ...
  for (i in 1:nrow(data)) {
    ...
    min_prob <- qnorm(0.975)/sqrt(size)
    prob_data <- # rescale_probabilities(
    ...
    tmp <- rescale_probabilities(
      matrix(ifelse(p_raw<1e-7, qnorm(0.975)/sqrt(size), p_raw),
             ncol=1) +
        use_house_effect*c(parse_house_effect(data[[i,'house_effect']])),
      qnorm(0.975)/sqrt(size))
    
    covariance_mat <- make_positive_definite(
      make_nonsingular(multinomial_covariance_mat(matrix(data = tmp, ncol=1), ifelse(is.na(data[[i, "predictive"]]), size, size)), size) +
...
pool_polls <- function(data, end_date) {
  if (nrow(data) == 1) {
    return(pool_single_poll(data, end_date))
  } else {
    return(pool_multiple_polls(data, end_date))
  }
}
```
...
norm_approx <- function(poll_df, rep, dem) {
  ...
  df <- poll_df %>%
    ...
    transmute(p_dem = !!democrat/100,
              ...
              p_undecided = ifelse(is.na(undecided),
                                   ifelse(is.na(other),
                                          0.75*(1 - !!democrat/100 - !!republican/100),
                                          (1 - !!democrat/100 - !!republican/100 - other/100)),
                                   undecided/100),
              p_third = ifelse(is.na(other),
                               ifelse(is.na(undecided),
                                      0.25*(1 - !!democrat/100 - !!republican/100),
  ...
  df  %>%
    nest_by(end_date) %>%
    mutate(data = list(pool_polls(data, end_date)))
}
```
...
full_kalman <- function(data, election_date) {
  end_T <- as.integer(election_date - min(data$end_date))
  xs <- list()
  ...
  covariance_matrices[[1]][3,4] <- abs(covariance_matrices[[1]][3,4])
  covariance_matrices[[1]][4,3] <- abs(covariance_matrices[[1]][4,3])
  residuals <- list()
  p <- list()
  ...
  for (i in 1:nrow(data)) {
    df <- data$data[[i]]
    end_dates <- c(end_dates, data$end_date)
    ...
    if (i > 1) {
      ...
      days_between_polls <- as.numeric(data$end_date[[i]] - data$end_date[[i-1]])
...
est_c_for_state <- function(state_abbrev) {
  tmp <- c(last(filter_state(state_abbrev, polls_2016, election_date_2016, dem=clinton)$prob))
...
prob_dem_win <- function(results, state_abbrev=NA, min_margin_of_victory=2, election_date=NA) {
  ...
  last_date <- last(sort(results$end_date))
  walk_distance <- ifelse(is.na(election_date), 1, sqrt(max(1, as.numeric(election_date-last_date))))
  ...
  for (state_name in unique(polls_2020$state)) {
    if (state_name != "--") {
      ...
      if (nrow(polls) > 0) {
        ...
        results[[state_name]] <- prob_dem_win(full_kalman(polls, election_date_2020), state_name, election_date=ifelse(is_nowcast, NA, election_date_2020))
      }
    }
  }
  results;
...
```{r}
percentage_support <- function(polls, election_date, dem, rep, end_date = today(), is_nowcast = F, remove_banned=F) {
  democrat <- enquo(dem)
  republican <- enquo(rep)
  ...
  for (state_name in unique(polls$state)) {
    if (state_name != "--") {
      ...
      if (nrow(polls) > 0) {
        probabilities <- last(full_kalman(normed_polls, election_date)$prob)
...
prob_2020 <- function(end_date = today(), is_nowcast = F, remove_banned=F) {
  percentage_support(polls_2020, election_date_2020, biden, trump, end_date, is_nowcast, remove_banned)
}
```

```{r}
prob_2016 <- function(end_date = today() - years(4), is_nowcast = F, remove_banned=F) {
  percentage_support(polls_2016, election_date_2016, clinton, trump, end_date, is_nowcast, remove_banned)
}
```

```{r}
p20 <- prob_2020()
kable(arrange(p20 %>% mutate(biden = democrat - republican), -biden))
```
...
plot_biden_win <- function(state_name, is_nowcast = F) {
  ...
  latest_poll_date <- last(sort(forecast$end_date))
...
outfile <- sprintf("../img/2020-plots/kalman-filter-%s.png",date_run)
