...
norm_approx <- function(poll_df, rep, dem) {
  ...
  poll_df %>%
    ...
    transmute(p_dem = !!democrat/100,
              ...
              state = state,
              mu_dem = sample_size*p_dem,
              mu_rep = sample_size*p_rep,
              mu_undecided = sample_size*p_undecided,
              mu_third = sample_size*p_third,
    ...
    summarize(p_dem = max(0, weighted.mean(p_dem, precision)),
              ...
              mu_dem = sum(mu_dem),
              mu_rep = sum(mu_rep),
              mu_undecided = sum(mu_undecided),
              mu_third = sum(mu_third),
              size = mu_dem + mu_rep + mu_undecided + mu_third,
              .groups = "drop")
}
```
...
ratings <- list(
  lean_dem = inv_logit(2),
  lean_rep = inv_logit(-2),
  likely_dem = inv_logit(3),
  likely_rep = inv_logit(-3),
