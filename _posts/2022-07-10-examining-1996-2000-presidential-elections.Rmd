...
foo92 <- left_join(la88 %>% mutate(r88 = republican,
  ...
  select(-Parish.y.y,-par,-land_area.y,-Parish.y,-Parish.x,-population.x) %>%
  rename(Parish = Parish.x.x,
...
Some cavalier calculations of voter turnout.

```{r}
summary(lm(t88 ~ 0 + white_vap + black_vap, foo92))
```
```{r}
summary(lm(d88 ~ 0 + white_voter + black_voter,
           foo92 %>% mutate(white_voter = 0.543397*white_vap,
                            black_voter = 0.519700*black_vap)))
```

```{r}
summary(lm(t92 ~ 0 + white_vap + black_vap, foo92))
```
```{r}
summary(lm(d92 -  black_voter ~ 0 + white_voter,
           foo92 %>% mutate(white_voter = 0.62364*white_vap,
                            black_voter = 0.52896*black_vap)))
```
