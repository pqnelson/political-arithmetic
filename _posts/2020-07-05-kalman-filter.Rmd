...
run_2020 <- function() {
  ...
  for (state_name in unique(polls_2020$state)) {
    if (state_name != "--") {
      polls <- norm_approx(polls_2020 %>% filter(state == state_name),
                           dem=biden, rep=trump);
      if (nrow(polls) > 0) {
...
okabe <- c("#aaa", "#222", "#ccc", "#afa", "#faa", "#aaf", "#fff")
...
ggplot() +
  geom_sf(sdf1, mapping = aes(fill = prediction),
          color = "#ffffff") +
  scale_fill_manual("Rating", values = c("#800000", "#ff0000", "#FF6347", "#DB7093",
                                         "#daa520", # "#a9a9a9",
                                         "#ADD8E6", "#00BFFF", "#1E90FF", "#00008B", "#DCDCDC"
                                         )) +
  labs(fill = "Rating") +
  coord_sf(datum = NA)
```
